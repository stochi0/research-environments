...
Supported domains:
  - library, fitness_gym, tech_support,
...
def download_tau2_data():
    """Download τ²-synth data from GitHub, if not present."""
    if os.path.exists(DATA_DIR) and os.path.exists(DATA_DIR / "tau2" / "domains"):
        return
    os.makedirs(DATA_DIR, exist_ok=True)
    temp_dir = Path("/tmp/tau2_synth")
    try:
        # clone repository
        subprocess.run(
            [
                ...
                "--branch",
                "synth",
                "https://github.com/mikasenghaas/tau2-synth.git",
                str(temp_dir),
            ],
            check=True,
            capture_output=True,
        ...
        if os.path.exists(src_data):
            ...
            print("Warning: Could not find data directory in tau2-synth repository")

    except subprocess.CalledProcessError as e:
        print(f"Warning: Failed to download tau2-synth data: {e}")
    finally:
...
class Tau2SynthMonitorRubric(vf.Rubric):
    """τ²-synth monitor rubric."""
    ...
    def num_errors(self, state: vf.State) -> float:
        tau2 = cast(Tau2SynthState, state["tau2"])
...
class Tau2SynthState(TypedDict):
    task: Task
    agent: LLMAgent
...
class Tau2SynthEnv(MultiTurnEnv):
    """
    τ²-synth environment supporting dual-control scenarios.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="tau2-synth")
        self.domain = domain
        ...
        rubric = self.create_tau2_rubric()
        super().__init__(
        ...
        self.add_rubric(Tau2SynthMonitorRubric())
    ...
    def create_tau2_rubric(self) -> vf.Rubric:
        ...
        async def evaluate_tau2_task(state, **kwargs) -> float:
            ...
            simulation = SimulationRun(
                id=f"{self.domain}_{task_id}_{datetime.now().isoformat()}",
                task_id=task_id,
