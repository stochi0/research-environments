...
A benchmark for abstract pattern recognition using the **RLM (Recursive Language Model)** pattern, where the model has access to a Python REPL to analyze patterns in the haystack.

This is the RLM variant of `patterned-needle-in-haystack`. The model must find "needle" lines hidden among "haystack" lines that differ only in their word-order pattern, but can use Python code to help analyze the data.

## RLM Features

The RLM environment provides:

- **Python REPL access**: Write code to analyze patterns programmatically
- **`extra_data` variable**: Access the haystack text in the REPL
- **`answer` variable**: Set `answer["content"]` and `answer["ready"] = True` to submit
- **`llm_batch()`**: Optional sub-LLM calls for delegating subtasks
...
```bash
# Install
vf-install patterned-needle-in-haystack-rlm

# Quick test (requires PRIME_API_KEY for sandbox)
vf-eval patterned-needle-in-haystack-rlm -n 5 -m gpt-5-mini

# With custom parameters
vf-eval patterned-needle-in-haystack-rlm \
    -n 100 \
    -m gpt-5-mini \
...
```python
from patterned_needle_in_haystack_rlm import load_environment
...
env = load_environment(
    ...
    # RLM options
    max_iterations=30,             # Maximum REPL turns
    sub_model=None,                # Model for sub-LLM calls (defaults to root)
    code_execution_timeout=120,    # Timeout for code execution (seconds)
    pip_install_packages="",       # Extra packages to install in sandbox
...
## Comparison with SingleTurnEnv Version

This RLM variant is designed to be **directly comparable** to the single-turn `patterned-needle-in-haystack` environment:

| Aspect | SingleTurnEnv | RLMEnv |
| ------ | ------------- | ------ |
| System prompts | Same hint levels | Same hint levels |
| Pattern generation | Same algorithm | Same algorithm |
| Scoring | Same exact match | Same exact match |
| Model access | Direct response | Python REPL |
| Input data | In prompt | In `extra_data` |

The key difference: in RLM mode, the model can write Python code to programmatically analyze patterns rather than reasoning in one shot.
